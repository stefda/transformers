...
def polylabel(polygon, precision=1.0, debug=False):
    ...
    minX = maxX = int(polygon[0][0][0])
    minY = maxY = int(polygon[0][0][1])
    for p in polygon[0][1:]:
        if p[0] < minX: minX = int(p[0])
        if p[1] < minY: minY = int(p[1])
        if p[0] > maxX: maxX = int(p[0])
        if p[1] > maxY: maxY = int(p[1])
    ...
    for x in range(minX, maxX - 1, int(cellSize)):
        for y in range(minY, maxY - 1, int(cellSize)):
